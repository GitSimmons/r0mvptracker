...
export const Settings: React.FC<{ columns: string[], toggleColumn: any }> = ({ columns, toggleColumn }: { columns: string[], toggleColumn: any }) => {
  ...
  return (
    <StyledSettings>
      ...
      <SettingsContainer visible={visible}>
        Visible Columns
        <ul>
          {allColumns.map((column) => <li key={column} onClick={(): void => toggleColumn(column)}><ColumnButton isVisible={columns.includes(column)}>{column}</ColumnButton></li>)}
        </ul>
      </SettingsContainer>
    </StyledSettings>
  )
